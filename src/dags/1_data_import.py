...
def load_data_postgres_vertica(table: str, operation_ts: str, count_date)->None:
     try:
          ...
          with vertica_python.connect(**vertica_conn_info) as connection:
               ...
               cur_vertica.execute(f"""DELETE FROM STV2023100639__STAGING.{ table } WHERE {operation_ts}::date='{count_date}'::date-1""")
               connection.commit()
               cur_vertica.copy(f'''COPY STV2023100639__STAGING.{table} FROM STDIN DELIMITER E'\t'  NULL AS 'null'  REJECTED DATA AS TABLE COPY_EX1_rej;''', input.getvalue())
