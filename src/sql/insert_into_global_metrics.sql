MERGE INTO STV2023100639__DWH.global_metrics AS gm
USING
    ...
    SELECT t.operation_id,
        ...
        t.status,
        t.transaction_type 
    FROM STV2023100639__STAGING.transactions AS t
    WHERE
        t.status='done' AND
    ...
    SELECT act.operation_id,
        ...
        act.status
    FROM all_currency_transactions AS act
    LEFT JOIN STV2023100639__STAGING.currencies as c ON act.currency_code=c.currency_code  and c.date_update::date=act.transaction_dt::date
